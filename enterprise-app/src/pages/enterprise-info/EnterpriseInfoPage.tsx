    ...
    <div>
      ...
      <Card>
        <Form form={form} layout="vertical" style={{ maxWidth: 520 }} onSubmit={handleSave}>
          ...
          <Form.Item label="统一社会信用代码" field="creditCode" rules={[{ required: true, match: /^\d{18}$/, message: '请输入18位统一社会信用代码' }]}>
            {editing ? <Input placeholder="18位" maxLength={18} /> : <span>{mockEnterprise.creditCode}</span>}
          </Form.Item>
          <Form.Item label="联系人姓名" field="contactName">
            {editing ? <Input placeholder="联系人姓名" maxLength={20} /> : <span>{mockEnterprise.contactName}</span>}
          </Form.Item>
          <Form.Item label="联系人手机号" field="contactPhone" rules={[{ match: /^1\d{10}$/, message: '请输入正确的手机号' }]}>
