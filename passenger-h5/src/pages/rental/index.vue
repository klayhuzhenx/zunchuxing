    ...
    <bottom-sheet v-model="showPackageSheet" title="选择车型与日期" :max-height="'90vh'">
      ...
      <view class="pkg-price-tag pkg-price-tag-prominent">
        <view class="pkg-price-wrap">
          <text v-if="hasDiscount" class="pkg-price-orig">¥{{ cars[pkgCarIdx].dayPrice.toLocaleString() }} / 天</text>
          <text class="pkg-price-text">¥{{ discountedUnit(cars[pkgCarIdx].dayPrice).toLocaleString() }} / 天 · 含 {{ cars[pkgCarIdx].kmPerDay }}km</text>
        </view>
        <view v-if="hasDiscount" class="pkg-price-badge">
          <text class="pkg-price-badge-text">{{ discountLabel }}</text>
        </view>
      </view>

      <view class="pkg-deposit-row">
        <text class="pkg-deposit-label">押金</text>
        <text class="pkg-deposit-val">{{ isEnterprise ? '企业免押' : `¥${(cars[pkgCarIdx].depositVehicle + cars[pkgCarIdx].depositViolation).toLocaleString()}` }}</text>
      </view>
...
type Car = {
  ...
  dayPrice: number;
  kmPerDay: number;
  depositVehicle: number; depositViolation: number;
...
const cars: Car[] = [
  {
    ...
    dayPrice: 1500,
    depositVehicle: 3000, depositViolation: 2000,
    kmPerDay: 100,
    features: [
  ...
  },
  {
    ...
    dayPrice: 1800,
    depositVehicle: 4000, depositViolation: 2000,
    kmPerDay: 100,
    features: [
  ...
  },
  {
    ...
    dayPrice: 2200,
    depositVehicle: 5000, depositViolation: 3000,
    kmPerDay: 100,
    features: [
...
/* 梯度折扣：随「租车天数」联动 */
const hasDiscount = computed(() => getRentalDiscountCoefficient(rentalDays.value) < 1);
const discountLabel = computed(() => getRentalDiscountLabel(rentalDays.value));
const discountedUnit = (price: number) =>
  hasDiscount.value ? calcRentalDiscountedUnit(price, rentalDays.value) : price;
const savingsText = computed(() =>
  calcRentalDiscountSavings(cars[pkgCarIdx.value].dayPrice, rentalDays.value).toLocaleString(),
);

const totalText = computed(() =>
  calcRentalDiscountedTotal(cars[pkgCarIdx.value].dayPrice, totalDays.value).toLocaleString()
);
...
.pkg-deposit-label { font-size: 14px; color: #4E5969; font-weight: 500; }
.pkg-deposit-val { font-size: 16px; color: #1D2129; font-weight: 700; }
