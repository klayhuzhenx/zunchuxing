        ...
        <view v-for="(o, i) in list" :key="i" class="card" :class="{ dim: o.s === 'cancelled' || o.s === 'completed' }" @click="detail(o)">
          ...
          <view class="ca">
            ...
            <template v-if="o.s === 'cancelled'">
              <view class="btn bo wf" @click.stop="redoOrder(o)">重新下单</view>
            </template>
          </view>
        </view>
      </view>
...
type S = 'unpaid'|'unpaid-extra'|'pending-unassigned'|'pending-assigned'|'pending-enroute'|'pending-pickup-undelivered'|'pending-pickup'|'ongoing'|'completed'|'cancelled';
type O = { t:'charter'|'rental'; s:S; route:string; date:string; car:string; drv:string; cd:string; ex:string; reason:string; ci:number; pi:string; dy:number; enroute?:{ dist:string; eta:string }; early?:{ pp:string; ac:string; pay:string } };
...
const orders = ref<O[]>([
  ...
  { t:'charter',s:'pending-enroute',route:'合肥南站 → 天鹅湖',date:'06-15 · 1天',car:'增程星辉行政版 · 尊荣高级',drv:'王师傅 · 沪B88888',cd:'',ex:'',reason:'',ci:0,pi:'a-h-pro',dy:1, enroute:{ dist:'2.3km', eta:'8分钟' } },
  ...
  { t:'charter',s:'ongoing',route:'合肥南站 → 滨湖万达',date:'06-15 至 06-16 · 2天',car:'增程星辉尊享版 · 尊享基础',drv:'王师傅 · 沪B88888',cd:'',ex:'',reason:'',ci:0,pi:'a-f-pro',dy:2,early:{pp:'5,950',ac:'6,661',pay:'711'} },
...
const sl = (o: O) => {
  const m: Record<string,string> = { unpaid:'待支付','unpaid-extra':'待结算','pending-unassigned':'待派车','pending-assigned':'待接驾','pending-enroute':'待接驾','pending-pickup-undelivered':'待取车','pending-pickup':'待取车',ongoing:'进行中',completed:'已完成',cancelled:'已取消' };
  return m[o.s]||o.s;
};
const sc = (s: S) => {
  if (s==='unpaid'||s==='unpaid-extra'||s==='cancelled') return 'er';
  if (s==='ongoing'||s==='completed'||s==='pending-pickup') return 'ok';
  if (s==='pending-assigned'||s==='pending-enroute') return 'bl';
  return 'wn';
};
...
const tmap: Record<string, S[]> = {
  all:[],
  unpaid:['unpaid'],
  'pending-start':['pending-unassigned','pending-assigned','pending-enroute','pending-pickup-undelivered','pending-pickup'],
  ongoing:['ongoing'],
  'unpaid-extra':['unpaid-extra'],
  completed:['completed'],
  cancelled:['cancelled'],
};
...
const detail = (o: O) => {
  ...
  const params = [
    ...
    `ex=${o.ex}`, `no=P2026061000${Math.min(orders.value.indexOf(o)+1,9)}`,
  ];
  if (o.t === 'charter') {
  ...
  if (o.early) params.push('earlyType=B');
...
.al.col { flex-direction: column; align-items: flex-start; }
...
.enroute-top { display: flex; align-items: center; gap: 8px; }
.enroute-text { font-size: 12px; color: #0057FF; opacity: 0.8; padding-left: 26px; }
