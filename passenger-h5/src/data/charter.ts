...
/**
 * 包车梯度折扣（§4.3 套餐选择 — 按下单天数递进让利）
 * 与运营端「包车梯度折扣」全局配置对齐；实际由后台返回，此处 mock。
 */
export interface CharterDiscountTier {
  fromDays: number;   // ≥ X 天
  toDays: number;     // < Y 天；-1 表示无上限
  coefficient: number; // 折扣系数，0.95 = 95 折
}

export const charterDiscountTiers: CharterDiscountTier[] = [
  { fromDays: 1, toDays: 2,  coefficient: 0.98 },
  { fromDays: 2, toDays: 6,  coefficient: 0.95 },
  { fromDays: 6, toDays: -1, coefficient: 0.90 },
];

/** 按下单天数匹配折扣系数；未命中门槛返回 1（原价） */
export const getDiscountCoefficient = (days: number): number => {
  const d = Math.max(1, Math.floor(days));
  const tier = charterDiscountTiers.find(
    (t) => d >= t.fromDays && (t.toDays === -1 || d < t.toDays),
  );
  return tier ? tier.coefficient : 1;
};

/** 折扣文案，如 "9.5折"；未达门槛返回 '' */
export const getDiscountLabel = (days: number): string => {
  const coef = getDiscountCoefficient(days);
  if (coef >= 1) return '';
  return `${(coef * 10).toFixed(1).replace(/\.0$/, '')}折`;
};

/** 折后总额（单价 × 天数 × 系数，整体四舍五入） */
export const calcDiscountedTotal = (price: number, days: number): number =>
  Math.round(price * days * getDiscountCoefficient(days));

/** 折后单价（仅展示用，单日四舍五入） */
export const calcDiscountedUnit = (price: number, days: number): number =>
  Math.round(price * getDiscountCoefficient(days));

/** 折扣节省金额 = 原总额 − 折后总额；未达门槛返回 0 */
export const calcDiscountSavings = (price: number, days: number): number =>
  price * days - calcDiscountedTotal(price, days);

/**
 * 租车梯度折扣——与包车同名函数对应，使用独立 rentalDiscountTiers。
 * 与运营端「租车梯度折扣」全局配置对齐。
 */
export const rentalDiscountTiers: CharterDiscountTier[] = [
  { fromDays: 1, toDays: 2,  coefficient: 0.98 },
  { fromDays: 2, toDays: 6,  coefficient: 0.95 },
  { fromDays: 6, toDays: -1, coefficient: 0.90 },
];

export const getRentalDiscountCoefficient = (days: number): number => {
  const d = Math.max(1, Math.floor(days));
  const tier = rentalDiscountTiers.find((t) => d >= t.fromDays && (t.toDays === -1 || d < t.toDays));
  return tier ? tier.coefficient : 1;
};

export const getRentalDiscountLabel = (days: number): string => {
  const coef = getRentalDiscountCoefficient(days);
  if (coef >= 1) return '';
  return `${(coef * 10).toFixed(1).replace(/\.0$/, '')}折`;
};

export const calcRentalDiscountedTotal = (price: number, days: number): number =>
  Math.round(price * days * getRentalDiscountCoefficient(days));

export const calcRentalDiscountedUnit = (price: number, days: number): number =>
  Math.round(price * getRentalDiscountCoefficient(days));

export const calcRentalDiscountSavings = (price: number, days: number): number =>
  price * days - calcRentalDiscountedTotal(price, days);
